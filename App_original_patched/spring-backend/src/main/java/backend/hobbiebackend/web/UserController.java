...
import backend.hobbiebackend.security.sanitized_entities.BusinessResponseDto;
import backend.hobbiebackend.security.sanitized_entities.HobbyResponseDto;
import backend.hobbiebackend.security.sanitized_entities.UserResponseDto;
...
public class UserController {
    ...
    @Autowired
    public UserController(UserService userService, PasswordEncoder passwordEncoder, NotificationService notificationService, JWTUtility jwtUtility, AuthenticationManager authenticationManager, HobbieUserDetailsService hobbieUserDetailsService,ModelMapper modelMapper) {
    ...
    */
    ...
    @GetMapping("/client")
    @Operation(summary = "show client-user information", security = @SecurityRequirement(name = "bearerAuth"))
    public UserResponseDto showUserDetails(String username) {
    	AppClient appClient = userService.findAppClientByUsername(username);
        return modelMapper.map(appClient, UserResponseDto.class);
    }
    ...
    */
    @GetMapping("/business")
    @Operation(summary = "Show business-user information", security = @SecurityRequirement(name = "bearerAuth"))
    public BusinessResponseDto getBusinessDetails(String username) {
        BusinessOwner businessOwner = userService.findBusinessByUsername(username);
        return modelMapper.map(businessOwner, BusinessResponseDto.class);
    }
    @PostMapping("/user")
