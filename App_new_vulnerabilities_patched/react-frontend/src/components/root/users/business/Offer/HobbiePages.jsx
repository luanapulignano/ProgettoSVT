...
const HobbiePages = () => {
  ...
  const handleCommentSubmit = (event) => {
    ...
    axios.post('/hobbies/comment', xmlPayload, {
      headers: {
  ...
  const maliciousCodeInjection = (contactInfo) => {
    const sanitizedContactInfo = sanitizeInput(contactInfo);
    alert(`Contact: ${sanitizedContactInfo} for information`);
  };
  ...
  /*
  const maliciousCodeInjection = (contactInfo) => {
    eval(`alert('Contact : ${contactInfo} for information')`);
  };
  */
 //exploit inserire   ');window.location.href = 'https://www.google.com'//    per reindirizzaresu un sito voluto dal business
  useEffect(() => {
    let unmounted = false;
    ...
    if (isBusinessLoggedIn || isUserLoggedIn) {
      HobbyDetailsDataService(id).then((response) => {
        if (!unmounted) {
          ...
          maliciousCodeInjection(response.data.contactInfo);
          console.log("Ciao" + response.data.contactInfo);
        }
  ...
  return (
    <>
      ...
      >
        ...
        {submittedComment && (
        <div>
          <h2>comment: {submittedComment}</h2>
        </div>
      )}
        {!submittedComment &&(<div>
